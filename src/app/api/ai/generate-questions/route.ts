...
export async function POST(request: Request) {
  try {
    ...
    const weightsList = Object.entries(capabilityWeights as Record<string, number>)
      .map(([k, v]) => `${k}: ${v}%`)
      .join(", ");

    const isInteractive = (questionTypes as string[]).includes("interactive");

    const deliveryContext =
      !isInteractive && deliveryFormats?.length > 0
        ? `\n交付格式要求：候选人需以【${(deliveryFormats as string[]).join("、")}】格式提交成果。`
        : "";

    const backgroundContext =
      !isInteractive && backgroundText?.trim()
        ? `\n背景信息：${backgroundText}`
        : "";

    const painPointsContext =
      isInteractive && painPoints?.trim() ? `\n业务现状/痛点：${painPoints}` : "";

    const objectiveContext =
      isInteractive && coreObjective?.trim()
        ? `\n核心业务目标：${coreObjective}`
        : "";

    const keyPointsInstruction =
      keyPoints?.length > 0
        ? `\n核心考查要点（题目须重点围绕以下要点设计）：\n${(keyPoints as string[]).map((p, i) => `${i + 1}. ${p}`).join("\n")}`
        : "";
...
题目类型: ${(questionTypes as string[]).join(", ")}
...
请以 JSON 格式返回，格式如下：
{
  "questions": [
    ...
    const questions = (parsed.questions as { seq: number; title: string; description: string }[]).map(
      (q, i, arr) => ({
        seq: q.seq,
        title: q.title,
        description: q.description,
        weight: i === 0 ? Math.floor(100 / arr.length) + (100 - Math.floor(100 / arr.length) * arr.length) : Math.floor(100 / arr.length),
      })
    );
