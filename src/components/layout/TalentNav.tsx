...
export function TalentNav({ username, isStudent }: { username?: string; isStudent?: boolean }) {
  ...
  return (
    <nav className="sticky top-0 z-50 px-4 sm:px-8 py-3 sm:py-4 flex justify-between items-center"
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        {isStudent && (
          <span className="hidden sm:inline text-xs text-emerald-400 font-medium px-2 leading-tight">
            学生身份已激活
          </span>
        )}
        <Link
          href="/register"
          className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium text-amber-300 border border-amber-500/40 hover:border-amber-400/80 hover:bg-amber-500/10 hover:text-amber-200 transition-all"
        ...
        <button onClick={handleLogout} className="hidden md:block text-xs text-slate-500 hover:text-slate-300 transition-colors">退出</button>
        <Link
          href="/talent/profile"
          className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 border-2 border-slate-700 hover:border-indigo-500 flex items-center justify-center text-xs font-bold text-white transition-colors"
          title="个人资料"
        >
